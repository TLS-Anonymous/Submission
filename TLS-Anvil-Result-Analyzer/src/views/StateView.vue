  ...
  <div class="analyzer">
    ...
    <template v-if="testMethod">
      <p style="max-width: 900px">
        ...
        <strong>Method:</strong> <span class="monospace">{{testMethod.ClassName.replace("anonymous.tlstest.suite.tests.", "")}}.{{testMethod.MethodName}}</span><br>
  ...
  methods: {
    ...
    downloadPcap(selectedCell, identifier, ev) {
      ...
      this.$http.get(`/testReport/${selectedCell.ContainerId}/testResult/${this.className}/${this.methodName}/${selectedCell.uuid}/pcap?download=1`, {responseType: 'blob'}).then((res) => {
        ...
        link.href = url
        link.download = `${identifier}_${this.methodName}_${this.className.replace('anonymous.tlstest.suite.tests.', '')}_${selectedCell.uuid}.pcap`
        link.click()
      }).then(() => {
