...
public class TLSExtensionForECC extends Tls12Test {
    ...
    @TlsTest(description = "Servers implementing ECC " 
    ...
    public void rejectsInvalidCurvePoints(ArgumentsAccessor argumentAccessor, WorkflowRunner runner) {
        ...
        WorkflowTrace workflowTrace = runner.generateWorkflowTraceUntilLastSendingMessage(WorkflowTraceType.HANDSHAKE, ProtocolMessageType.CHANGE_CIPHER_SPEC);
        ECDHClientKeyExchangeMessage clientKeyExchangeMessage = (ECDHClientKeyExchangeMessage) WorkflowTraceUtil.getFirstSendMessage(HandshakeMessageType.CLIENT_KEY_EXCHANGE, workflowTrace);
    ...
    @ComplianceCategory(SeverityLevel.HIGH)
    @Tag("new")
    public void leavesPublicKeyUnsignedInAnon(ArgumentsAccessor argumentAccessor, WorkflowRunner runner) {
        Config config = getPreparedConfig(argumentAccessor, runner);
        WorkflowTrace workflowTrace = runner.generateWorkflowTrace(WorkflowTraceType.HELLO);
        runner.execute(workflowTrace, config).validateFinal(i -> {
            Validator.executedAsPlanned(i);
            ECDHEServerKeyExchangeMessage serverKeyExchange = workflowTrace.getFirstReceivedMessage(ECDHEServerKeyExchangeMessage.class);
            assertEquals("Server provided a non-empty signature field in Server Key Exchange message", (long)0, (long)serverKeyExchange.getSignatureLength().getValue());
        });
    }
    
    public boolean isEcdheAnonCipherSuite(CipherSuite cipherSuite) {
        return cipherSuite.isAnon() && cipherSuite.isEphemeral() && AlgorithmResolver.getKeyExchangeAlgorithm(cipherSuite).isEC();
    }
